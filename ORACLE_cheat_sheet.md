...
## Procedure for granting select on all your view to another user
```sql
begin
    for x in (select * from all_views where owner = 'username1')
    loop
        execute immediate 'GRANT SELECT ON '||x.username1||'.'|| x.view_name || ' to ' || 'username2';
    end loop;
end;
```
...
begin
    ...
    loop
        execute immediate 'GRANT SELECT ON '||x.username1||'.'|| x.table_name || ' to ' || 'username2';
    end loop;
end;
```
## privilege levels  
